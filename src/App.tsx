    ...
    <div className="flex min-h-screen bg-[#F5F2EA]">
          ...
          {NAV.map(([id, label, Ic]) => {
            ...
            return (
              <button key={id} onClick={() => setView(id)}
                ...
                style={{ color: on ? "#0B0E24" : "#69728A", background: on ? "rgba(11,14,36,.06)" : "transparent" }}>
                <Ic size={18} color={on ? "#D8B25A" : undefined} /> {label}
              </button>
            );
          })}
        </nav>
        <div className="absolute bottom-4 left-4 right-4">
          <div className="bg-[#F5F2EA] rounded-xl p-3.5">
            <p className="text-[12px] text-[#69728A] font-semibold mb-2">Overall readiness</p>
            <div className="flex items-center gap-2.5">
              <Ring value={overall.avg} size={40} stroke={4} color="#2FB68A" />
      ...
      <div className="flex-1 min-w-0">
        ...
        <header className="h-[62px] border-b border-[#E7EAF3] sticky top-0 z-10 flex items-center gap-3.5 px-6 no-print" style={{ background: "rgba(255,255,255,.8)", backdropFilter: "blur(8px)" }}>
          ...
          <div className="hidden md:flex items-center gap-2 flex-1 max-w-[420px] bg-[#F5F2EA] border border-[#E7EAF3] rounded-xl px-3 py-2">
          ...
          <div className="w-9 h-9 rounded-full grid place-items-center text-white text-[13px] font-bold" style={{ background: "#0B0E24" }}>RS</div>
        </header>
        ...
        <div className="md:hidden flex gap-1 overflow-x-auto px-4 py-2 border-b border-[#E7EAF3] bg-white no-print">
          {NAV.map(([id, label, Ic]) => (
            <button key={id} onClick={() => setView(id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[13px] font-semibold whitespace-nowrap"
              style={{ color: view === id ? "#0B0E24" : "#69728A", background: view === id ? "rgba(11,14,36,.06)" : "transparent" }}>
              <Ic size={15} /> {label}
            </button>
          ))}
        </div>
      ...
      <AnimatePresence>
        {toastMsg && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }}
            ...
            <CheckCircle2 size={17} color="#2FB68A" /> {toastMsg}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
